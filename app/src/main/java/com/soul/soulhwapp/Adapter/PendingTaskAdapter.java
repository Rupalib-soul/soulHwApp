...
public class PendingTaskAdapter extends RecyclerView.Adapter<PendingTaskAdapter.MyViewHolder> {
    private ArrayList<TaskModel> pendingTaskList;

    public PendingTaskAdapter(ArrayList<TaskModel> pendingTaskList) {
        this.pendingTaskList = pendingTaskList;
    }


    @Override
    public PendingTaskAdapter.MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {

        View itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.task_list_items, parent, false);
        return new PendingTaskAdapter.MyViewHolder(itemView);
    }
    ...
    @Override
    public void onBindViewHolder(PendingTaskAdapter.MyViewHolder holder, int position) {

        TaskModel tm = pendingTaskList.get(position);
